...
Security fixes are focused on the latest public release of Uangku.
...
Uangku is an offline-first Android app for personal finance tracking.
...
Do not treat Uangku as a replacement for device-level security, encrypted storage, or a password manager.
...
Backup and restore are user-controlled. Uangku validates backup files before restore, but users should keep backup files in a safe place and avoid sharing them publicly.
...
```powershell
Get-FileHash .\Uangku-vX.X.X-arm64.apk -Algorithm SHA256
```

Uangku does not claim to be "100% secure". Security is handled as an ongoing maintenance responsibility.
