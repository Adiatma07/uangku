...
class _AddTransactionSheetState extends ConsumerState<AddTransactionSheet> {
        ...
        child: Column(
          ...
          children: [
            ListTile(
              ...
              subtitle: const Text('Screenshot bukti transfer, struk digital, dll.'),
              onTap: () =>
                  Navigator.pop(context, ReceiptImageSource.gallery),
            ),
            ListTile(
              leading: const Icon(Icons.photo_camera_outlined),
              title: const Text('Foto Struk'),
              subtitle: const Text('Ambil foto struk kertas langsung.'),
              onTap: () => Navigator.pop(context, ReceiptImageSource.camera),
            ),
  ...
  void _applyParseResult(ReceiptParseResult result) {
    if (!result.hasAnyMatch) {
      _showNotice(
        'Teks tidak terbaca jelas. Coba foto/screenshot yang lebih tajam, atau isi manual.',
        _NoticeTone.warning,
      );
      return;
    }
    ...
    setState(() {
      if (result.amount != null) {
        _amountDigits = result.amount!.toString();
      }
      if (result.date != null) {
        final hour = result.hour ?? _selectedDate.hour;
        final minute = result.minute ?? _selectedDate.minute;
        _selectedDate = DateTime(
          result.date!.year,
          result.date!.month,
          result.date!.day,
    ...
    _showNotice(
      'Terisi otomatis dari hasil scan. Periksa dulu sebelum simpan ya.',
      _NoticeTone.info,
    );
  }

  Future<void> _scanReceipt(ReceiptImageSource source) async {
    final repository = ref.read(receiptScannerRepositoryProvider);

    setState(() => _scanning = true);
    try {
      final File? image = await repository.pickImage(source);
      if (image == null) {
        setState(() => _scanning = false);
        return;
      }

      final result = await repository.extractFromImage(image);
      if (!mounted) return;
      _applyParseResult(result);
    } catch (_) {
      if (!mounted) return;
      _showNotice(
        'Gagal membaca gambar. Coba lagi atau isi manual.',
        _NoticeTone.warning,
      );
    } finally {
